...
def draw(locationsRawList: list, output_path: str, file_name: str) -> None:
    ...
    content = 'map.centerAndZoom(new BMapGL.Point(%s, %s), 4)\n' % (location_center_lng, location_center_lat)
    for i in locationsRawList:
        # content += 'AddLabel(map, "%s", %s, %s)\n' % (i[2], i[0], i[1])
        content += 'AddPathPoint(path, %s, %s)\n' % (i[0], i[1])
        content += 'AddPoint(map, "%s", "%s", %s, %s)\n' % (i[2], i[3], i[0], i[1])
    template = ''
    ...
    for k in rawData['Hops']:
        for j in k:
            if j['Success']:
                ...
                if j['Geo']['city'] != '':
                    tmpCity = j['Geo']['city']
                coordinatesList.append([j['Geo']['lat'], j['Geo']['lng'], tmpCity, j['Geo']['owner']])
                break
